...
# Copy jaas config file template to conf
COPY jaas.conf /opt/zookeeper/conf
...
RUN set -x \
    ...
    && mkdir -p $ZK_DATA_DIR $ZK_DATA_LOG_DIR $ZK_LOG_DIR /usr/share/zookeeper /tmp/zookeeper /usr/etc/ /opt/$ZK_DIST/security \
