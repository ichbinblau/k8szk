...
JAAS_FILE="$ZK_CONF_DIR/jaas.conf"
HOST=`hostname -s`
DOMAIN=`hostname -d`
...
function create_java_env() {
    ...
    echo "JVMFLAGS=\"-Djava.security.auth.login.config=$ZK_CONF_DIR/jaas.conf\"" >> $JAVA_ENV_FILE
    echo "Wrote JVM configuration to $JAVA_ENV_FILE"
}

function create_jaas() {
    sed -i "s/HOST/$HOST.$DOMAIN/g" $JAAS_FILE   
    echo "Update Jaas configuration to $JAAS_FILE"
}

validate_env && create_config && create_log_props && create_data_dirs && create_java_env && create_jaas
